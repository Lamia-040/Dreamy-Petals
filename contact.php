...
// If user is logged in, store ID, else keep it null/empty
...
if (isset($_POST['send'])) {
    ...
    if (mysqli_num_rows($select_message) > 0) {
        ...
        // If user not logged in, save NULL in user_id column
        $uid = $user_id ? "'$user_id'" : "NULL";

        mysqli_query($conn, "INSERT INTO `message`(user_id, name, email, number, message) 
            VALUES($uid, '$name', '$email', '$number', '$msg')") or die('query failed');
